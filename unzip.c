...
void read_param(int *pwidth,int *pheight,int *prange,int *pnbColors,FILE *zip_file){
	...
	*pwidth=width_byte*255;//we use a pointer to get the param in the main
...
int read_same_block(unsigned char byte,PPM_IMG* zipped,int i,int *pj,int previous_pixel_value,int width){
	int counter;
	counter=byte-significant_bit_same+counter_increment;// get the exact counter
	
	for(int k=0;k<counter;k++){
		ppmWrite(zipped,*pj,i,previous_pixel_value);//write the previous pixel value 
	    	*pj=*pj+1;
	    }
        *pj=*pj-1;
...
int read_index_block(unsigned char byte,int cache[],int i,int j,PPM_IMG* zipped){
   	unsigned char index=byte;
    	int pix;
	pix=cache[index];//get the value of the pixel stored in the cache
	ppmWrite(zipped,j,i,pix);//write it
	return(pix);
}
...
int read_diff_block(unsigned char byte,int previous_pixel_value,int i,int j,PPM_IMG* zipped){
    	...
    	//get the diff in the diff block between the current and previous pixel of every color 
...
int read_luma_block(FILE *zip_file,unsigned char byte,int i,int j,PPM_IMG* zipped,int previous_pixel_value){
	...
	//get the others colors with the green difference
...
int read_rgb_block_pgm(FILE *zip_file,int j, int i,PPM_IMG* zipped){
	unsigned char grey;
	int pix;
	//get the color of value of the pixel after the rgb block
...
int read_luma_block_pgm(FILE *zip_file,unsigned char byte,int i,int j,PPM_IMG* zipped,int previous_pixel_value){
	unsigned char diff_grey;
	byte=byte-significant_bit_luma;
	diff_grey=byte-offset_diff_grey;
    	unsigned char grey;
	grey=blue(previous_pixel_value)+diff_grey;
...
void unzip(char **path){
	...
	path_exit=path[2];//path relative to the ppm img
	if (path_exit==NULL){
	...
	prange=&range;
	pnbColors=&nbColors;
 	//get the param of the ppm img
	read_param(pwidth,pheight,prange,pnbColors,zip_file);
	//create a ppm img structure
	PPM_IMG *zipped;
	zipped=ppmNew(width,height,range,nbColors);
	//ppm img traitment
	if(nbColors==3){
