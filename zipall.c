...
diff_grey=significant_bit_luma+diff_grey+offset_diff_green;// diff grey and diff green have a same offset
}


void block_rgb_grey(int pixel_value,FILE *zipped,unsigned char block_rgb_bit){
	unsigned char grey_byte=red(pixel_value);    	
...
unsigned char index;
int grey=red(previous_pixel_value);
int previous_grey;
	...
	for (i=0;i<height;i++){
		...
		pixel_value=ppmRead(img_entree,j,i);//get the current pixel value
		grey=red(pixel_value);
		index=(3*grey+5*grey+7*grey)%64;
		previous_grey=red(previous_pixel_value);
		//calculate diff
